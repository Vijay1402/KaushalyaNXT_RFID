...
String encodeTagData(TagData tag) {
  ...
  // 0–11: Tree ID (12 bytes) stored here so apps can write a custom Tree ID.
  // Store the `treeId` as UTF-8 (truncated to 12 bytes). Remaining reserved
  // bytes up to 16 remain zero.
  final treeIdUtf8 = utf8.encode(tag.treeId);
  final treeStore = Uint8List(16);
  final copyLen = min(treeIdUtf8.length, 12);
  if (copyLen > 0) {
    treeStore.setRange(0, copyLen, treeIdUtf8.sublist(0, copyLen));
  }
  bytes.setRange(0, 16, treeStore);
...
TagData decodeTagData(
  ...
  // Tree ID is read from USER[0..11] if present; otherwise EPC is used.
  ...
  String treeIdFromUser = '';
  if (treeSlice.isNotEmpty) {
    try {
      treeIdFromUser = utf8.decode(treeSlice, allowMalformed: true);
    } catch (_) {
      treeIdFromUser = '';
    }
  }
